...
module.exports = async ({ category = '' } = {}) => {
    ...
    const deals = MOCK_DATA.filter((item) => {
    ...
    const summary = filter
        ? `Found ${deals.length} current deal${deals.length === 1 ? '' : 's'} in ${filter}.`
        : `Found ${deals.length} current deal${deals.length === 1 ? '' : 's'} across all categories.`;
